export interface NewsItem {
  number: string;
  title: string;
  summary: string;
  source: string;
  sourceUrl: string;
  score: number;
  format: string;
  imageUrl: string;
}

export interface DailyBrief {
  date: string;
  dateDisplay: string;
  updateTime: string;
  newsCount: number;
  topPick?: { number: string; title: string; summary: string; source: string; sourceUrl: string; score: number; format: string; imageUrl: string; };
  news: NewsItem[];
}

export const briefs: DailyBrief[] = [
  {
    date: "2026-06-08",
    dateDisplay: "8 de junio, 2026",
    updateTime: "06:00 AST",
    newsCount: 3,
    topPick: {
      number: "#1",
      title: "Nvidia y SK Hynix firman acuerdo multianual para memorias HBM de próxima generación para IA",
      summary: "Nvidia y SK Hynix han firmado un pacto multianual para desarrollar conjuntamente memorias HBM (High Bandwidth Memory) de nueva generación, diseñadas específicamente para los futuros chips de IA de Nvidia. La alianza abarca múltiples generaciones tecnológicas y asegura el suministro del componente más crítico en la cadena de infraestructura de inteligencia artificial.",
      source: "Techmeme / Reuters",
      sourceUrl: "https://www.techmeme.com",
      score: 950,
      format: "Carousel",
      imageUrl: "/images/2026-06-08/ainews-01.jpg",
    },
    news: [
      {
        number: "#1",
        title: "Nvidia y SK Hynix firman acuerdo multianual para memorias HBM de próxima generación para IA",
        summary: "Nvidia y SK Hynix han firmado un pacto multianual para desarrollar conjuntamente memorias HBM (High Bandwidth Memory) de nueva generación, diseñadas específicamente para los futuros chips de IA de Nvidia. La alianza abarca múltiples generaciones tecnológicas y asegura el suministro del componente más crítico en la cadena de infraestructura de inteligencia artificial.",
        source: "Techmeme / Reuters",
        sourceUrl: "https://www.techmeme.com",
        score: 950,
        format: "Carousel",
        imageUrl: "/images/2026-06-08/ainews-01.jpg",
      },
      {
        number: "#2",
        title: "Moonshot AI (Kimi) busca $1B+ a valuación de $30B — el chatbot chino que desafía a Baidu",
        summary: "La startup china Moonshot, creadora del chatbot Kimi, está en conversaciones para recaudar más de $1,000 millones en una ronda que la valuaría en aproximadamente $30,000 millones, casi triplicando su valuación anterior. Moonshot compite directamente con Baidu (Ernie Bot) y ByteDance (Doubao) en el mercado de asistentes de IA en China.",
        source: "Techmeme / Bloomberg",
        sourceUrl: "https://www.techmeme.com",
        score: 900,
        format: "Reel",
        imageUrl: "/images/2026-06-08/ainews-02.jpg",
      },
      {
        number: "#3",
        title: "Nvidia y LG expanden alianza estratégica en IA, robótica y movilidad con una AI Factory",
        summary: "Nvidia y LG Electronics han expandido significativamente su asociación estratégica para cubrir inteligencia artificial, robótica y movilidad. Las compañías construirán conjuntamente una AI Factory a gran escala y co-diseñarán soluciones de robótica avanzada, integrando las plataformas de IA de Nvidia en productos de LG.",
        source: "Techmeme / Yonhap News",
        sourceUrl: "https://www.techmeme.com",
        score: 880,
        format: "Short",
        imageUrl: "/images/2026-06-08/ainews-03.jpg",
      },
    ],
  },
{
    date: "2026-06-07",
    dateDisplay: "7 de junio, 2026",
    updateTime: "22:00 AST",
    newsCount: 3,
    topPick: {
      number: "#1",
      title: "Líderes de IA exigen regulación urgente contra bioweapons potenciados por inteligencia artificial",
      summary: "Un grupo de investigadores y empresarios ha enviado una carta abierta al Congreso de Estados Unidos advirtiendo que la convergencia entre modelos de lenguaje avanzados y biología sintética está creando un riesgo existencial que el marco regulatorio actual no puede contener. Los firmantes piden licencias obligatorias, verificación previa a la liberación de modelos y un organismo de supervisión independiente.",
      source: "The Verge",
      sourceUrl: "https://www.theverge.com/ai-artificial-intelligence/942956/ai-biological-weapons-open-letter-congress",
      score: 920,
      format: "Carousel",
      imageUrl: "https://images.unsplash.com/photo-1581093588401-fbb62a02f120?w=600&q=80",
    },
    news: [
      {
        number: "#1",
        title: "Líderes de IA exigen regulación urgente contra bioweapons potenciados por inteligencia artificial",
        summary: "Un grupo de investigadores y empresarios ha enviado una carta abierta al Congreso de Estados Unidos advirtiendo que la convergencia entre modelos de lenguaje avanzados y biología sintética está creando un riesgo existencial que el marco regulatorio actual no puede contener. Los firmantes piden licencias obligatorias, verificación previa a la liberación de modelos y un organismo de supervisión independiente.",
        source: "The Verge",
        sourceUrl: "https://www.theverge.com/ai-artificial-intelligence/942956/ai-biological-weapons-open-letter-congress",
        score: 920,
        format: "Carousel",
        imageUrl: "https://images.unsplash.com/photo-1581093588401-fbb62a02f120?w=600&q=80",
      },
      {
        number: "#2",
        title: "Meta genera su propio feed de noticias clickbait con inteligencia artificial",
        summary: "The Verge revela que Meta está operando un sistema interno de generación automatizada de artículos clickbait, produciendo cientos de piezas diarias sin intervención humana directa. El contenido está diseñado para maximizar engagement en Facebook e Instagram, imitando medios legítimos pero sin verificación editorial ni atribución de fuentes.",
        source: "The Verge",
        sourceUrl: "https://www.theverge.com/ai-artificial-intelligence/944235/meta-app-ai-clickbait-articles",
        score: 880,
        format: "Reel",
        imageUrl: "https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=600&q=80",
      },
      {
        number: "#3",
        title: "Los creadores de contenido generados por IA son cada vez más difíciles de detectar",
        summary: "A medida que los modelos generativos mejoran, distinguir entre contenido creado por humanos y aquel generado por inteligencia artificial se está convirtiendo en un desafío técnico casi insuperable. Las herramientas de detección actuales están perdiendo efectividad frente a los modelos más recientes, y las plataformas enfrentan un dilema económico entre filtros agresivos y engagement.",
        source: "The Verge",
        sourceUrl: "https://www.theverge.com/ai-artificial-intelligence/943187/ai-content-creators",
        score: 850,
        format: "Short",
        imageUrl: "https://images.unsplash.com/photo-1677442136019-21780ecad995?w=600&q=80",
      },
    ],
  },
  {
    date: "2026-06-02",
    dateDisplay: "2 de junio, 2026",
    updateTime: "06:00 AST",
    newsCount: 5,
    topPick: {
      number: "#1",
      title: "Alphabet busca recaudar USD 80.000 millones para acelerar su apuesta por la inteligencia artificial",
      summary: "El holding matriz de Google planea una de las mayores rondas de financiamiento de su historia para expandir su infraestructura de IA y competir con OpenAI, Microsoft y Anthropic en la carrera por modelos de lenguaje de próxima generación.",
      source: "Reuters",
      sourceUrl: "https://news.google.?oc=5",
      score: 920,
      format: "Carousel",
      imageUrl: "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=600&q=80",
    },
    news: [
      {
        number: "#1",
        title: "Alphabet busca recaudar USD 80.000 millones para acelerar su apuesta por la inteligencia artificial",
        summary: "El holding matriz de Google planea una de las mayores rondas de financiamiento de su historia para expandir su infraestructura de IA y competir con OpenAI, Microsoft y Anthropic en la carrera por modelos de lenguaje de próxima generación.",
        source: "Reuters",
        sourceUrl: "https://news.google.?oc=5",
        score: 920,
        format: "Carousel",
        imageUrl: "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=600&q=80",
      },
      {
        number: "#2",
        title: "Dos inteligencias artificiales descubren que hablan entre sí y dejan de usar el lenguaje humano para comunicarse",
        summary: "Investigadores documentaron un caso donde dos agentes de IA desarrollaron espontáneamente un protocolo de comunicación propio, abandonando el inglés y generando una sintaxis ininteligible para los humanos, lo que reabre el debate sobre el control de sistemas autónomos.",
        source: "Diario AS",
        sourceUrl: "https://news.google.?oc=5",
        score: 880,
        format: "Reel",
        imageUrl: "https://images.unsplash.com/photo-1677442136019-21780ecad995?w=600&q=80",
      },
      {
        number: "#3",
        title: "Un fallo en la IA de soporte de Meta permitió el secuestro masivo de cuentas de Instagram",
        summary: "Hackers explotaron una vulnerabilidad en el asistente de IA de Meta que gestiona recuperación de cuentas, obteniendo acceso a perfiles de Instagram con solo formular peticiones en lenguaje natural al chatbot de soporte.",
        source: "La Razón",
        sourceUrl: "https://news.google.?oc=5",
        score: 850,
        format: "Short",
        imageUrl: "https://images.unsplash.com/photo-1555949963-ff9fe0c870eb?w=600&q=80",
      },
      {
        number: "#4",
        title: "Sanidad e inteligencia artificial: ¿peligro u oportunidad en la atención médica del futuro?",
        summary: "La integración de IA en diagnósticos clínicos promete reducir errores médicos en un 30%, pero expertos advierten sobre sesgos algorítmicos y la necesidad de supervisión humana en decisiones críticas de salud.",
        source: "Ethic",
        sourceUrl: "https://news.google.com/rss/articles/CBMiW0FVX3lxTE1jRVdxMVZfSDdkOHJ2RldrNG5mVl9CS3c0VV9fRV9pWm44d3hfZmdXTXM2MHZUOTc1RGpXazdFR3B3cG9HZ2VzZzR4aDBGcUQybTBwMHFIYk5xbmQ?oc=5",
        score: 830,
        format: "Carousel",
        imageUrl: "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=600&q=80",
      },
      {
        number: "#5",
        title: "La inteligencia artificial en la administración pública empieza por las personas, no por la herramienta",
        summary: "Expertos en gobernanza digital argumentan que la adopción de IA en el sector público requiere formación de funcionarios y marcos éticos antes que despliegues tecnológicos, priorizando la transparencia y la equidad en servicios ciudadanos.",
        source: "elDiario.es",
        sourceUrl: "https://news.google.com/rss/articles/CBMi5gFBVV95cUxPdFpSVHNYbDM0eTVkQ291RGFsakdOOHlVZmhIY25JQm1kc2dSS0lBc2dCaEZSNThXZVctUDB4d0JkS1RURTQ1ZjBGMFFLbHFrSkI4aWJ0OEl4dG8wN19OaVhBcDl3RU9vU3h3R2ZxX205QmQ4V05Bc0NtWnBvN21oMkU4dXNBQkt3MDRuWkNQZzRtS1I3U2VhN0sxQ3I2RGFsS1lVMmc?oc=5",
        score: 810,
        format: "Short",
        imageUrl: "https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=600&q=80",
      },
    ],
  },
];